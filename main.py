...
import numpy as np
import cv2
from cv2 import FONT_ITALIC, resize, imread, imshow, namedWindow, WINDOW_NORMAL, \
    setMouseCallback, createTrackbar, putText, EVENT_LBUTTONDOWN, EVENT_MOUSEMOVE, \
    EVENT_LBUTTONUP, waitKey, getTrackbarPos, line
from model import *
...
namedWindow("Object Removal", WINDOW_NORMAL) 
setMouseCallback('Object Removal', mouse_callback)
...
createTrackbar('Pen Size', 'Object Removal', 1, 10, lambda x: x)
# ^ Widget for pensize
...
while(True):
    ...
    # Adjust pen size
    stroke_size = getTrackbarPos('Pen Size', 'Object Removal')

cv2.imwrite(sys.argv[1].replace('uploads', 'outputs'), input_image_masked)
